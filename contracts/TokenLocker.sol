...
contract TokenLocker is ITokenController, IForwarder, AragonApp {
    ...
    uint256 public constant CLAIM_TOKENS = 1; //Number of tokens received when claiming for contributing to the crowdsale
    ...
    */
    function claim(uint16 _day) external {
      ...
      token.generateTokens(msg.sender, CLAIM_TOKENS); // minime.generateTokens() never returns false
    ...
    */
    function lock(uint256 _lockTime) external {
      ...
      uint expiry;
      uint claimed = 0;
      if(claimedToken[msg.sender]){ claimed = CLAIM_TOKENS;}
    ...
    */
    ...
    returns (uint256, uint256){
      ...
      uint lockDiff = lockVal.sub(lockAmount[msg.sender]);
      lockAmount[_user] = lockVal;
      uint claimed = 0;
      if(claimedToken[_user]){ claimed = CLAIM_TOKENS;}
