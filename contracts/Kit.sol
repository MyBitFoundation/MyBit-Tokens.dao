...
contract Kit is KitBase {
    ...
    function setupDAO(Kernel _dao, ACL _acl) internal {
      ...
      MyBitTokenLocker tokenLocker = MyBitTokenLocker(_dao.newAppInstance(tokenLockerAppId, latestVersionAppBase(tokenLockerAppId)));
      ...
      tokenLocker.initialize(token, address(erc20), address(0), address(identity), lockAmounts, lockIntervals, tokenIntervals);
      //voting.initialize(token, 50 * PCT, 20 * PCT, 1 days);
      identity.initialize(whitelist);
    ...
    function setupPermissions(Kernel _dao, ACL _acl, /*Voting _voting,*/ MyBitTokenLocker _tokenLocker, Vault _vault, Finance _finance, address _root) internal{
